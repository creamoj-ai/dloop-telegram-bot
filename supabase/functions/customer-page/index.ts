...
serve(async (req: Request) => {
  console.log("[customer-page] Request received:", req.method, req.url);
  ...
  console.log("[customer-page] Token extracted:", token);
...
async function handlePost(token: string, req: Request, supabase: any): Promise<Response> {
  ...
  const deliverySlot = body.delivery_slot?.toString().trim() || ""; // Fascia oraria dal frontend
  ...
  const updateData: Record<string, unknown> = {
    ...
    delivery_slot: deliverySlot || null, // Fascia oraria scelta dal cliente
